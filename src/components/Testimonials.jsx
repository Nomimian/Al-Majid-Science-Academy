...
const testimonials = [
  ...
  { quote: 'My daughter\'s confidence has grown tremendously since joining. The co-curricular programme here is simply wonderful.', name: 'Fatima Noor', role: 'Parent of Grade 6 Student', initials: 'FN' },
  { quote: 'The Quran and Islamic studies programme is exceptional. My son has memorised multiple surahs and his character has improved remarkably.', name: 'Usman Butt', role: 'Parent of Grade 5 Student', initials: 'UB' },
    ...
    <section ref={ref} className="bg-white py-20 md:py-28 px-[5%] overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          {/* Mobile single card */}
          <div className="block md:hidden relative min-h-[260px]">
            <AnimatePresence mode="wait" custom={direction}>
              <motion.div
                ...
                className="bg-[#F8F6F0] rounded-2xl p-7 border border-[rgba(15,31,92,0.06)] relative"
              >
                <div className="absolute top-4 right-6 text-7xl text-[#D4A017] opacity-10 font-[Georgia,serif] leading-none">"</div>
                <div className="flex gap-0.5 mb-4">{[...Array(5)].map((_, i) => <span key={i} className="text-[#D4A017] text-base">★</span>)}</div>
                <p className="text-[#3A4875] text-[15px] leading-[1.8] mb-6 italic">"{testimonials[current].quote}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#0F1F5C] to-[#1B2A6B] flex items-center justify-center text-[#D4A017] text-sm font-extrabold shrink-0">
                    {testimonials[current].initials}
                  </div>
                  <div>
                    <div className="text-[#0F1F5C] font-bold text-sm">{testimonials[current].name}</div>
                    <div className="text-[#6B7BAD] text-xs">{testimonials[current].role}</div>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
          ...
          <div className="hidden md:grid grid-cols-3 gap-7">
            {getVisible().map((idx, pos) => (
              <motion.div
                key={idx}
                animate={{ opacity: pos === 1 ? 1 : 0.55, scale: pos === 1 ? 1 : 0.95 }}
                transition={{ duration: 0.4 }}
                className="bg-[#F8F6F0] rounded-2xl p-8 border border-[rgba(15,31,92,0.06)] relative cursor-pointer"
                onClick={() => goTo(idx)}
              >
                <div className="absolute top-4 right-6 text-7xl text-[#D4A017] opacity-10 font-[Georgia,serif] leading-none">"</div>
                <div className="flex gap-0.5 mb-4">{[...Array(5)].map((_, i) => <span key={i} className="text-[#D4A017] text-base">★</span>)}</div>
                <p className="text-[#3A4875] text-[15px] leading-[1.8] mb-6 italic">"{testimonials[idx].quote}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#0F1F5C] to-[#1B2A6B] flex items-center justify-center text-[#D4A017] text-sm font-extrabold shrink-0">
                    {testimonials[idx].initials}
                  </div>
                  <div>
                    <div className="text-[#0F1F5C] font-bold text-sm">{testimonials[idx].name}</div>
                    <div className="text-[#6B7BAD] text-xs">{testimonials[idx].role}</div>
                  </div>
                </div>
              </motion.div>
            ))}
